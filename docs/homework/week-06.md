...
4. If Tavily is configured, run one real-time query:
   - `TAVILY_API_KEY=... PYTHONPATH=src .venv/bin/python -m compliance_bot.graph.workflow --manifest-path artifacts/corpus/manifest-week-02-v1.json --question "What is the latest public guidance on expense reimbursement approvals?" --jurisdiction US --policy-scope expense --embedding-provider siliconflow --rerank-provider siliconflow --llm-provider siliconflow --tool-timeout-ms 5000 --exception-log-path docs/policies/sanitized/exception-log-week-06.json`
