...
1. For practical retrieval tasks, default to hosted providers for embeddings and rerank (SiliconFlow-first) behind adapters.
2. Keep model/provider settings in typed config loaders, not hard-coded in chain logic.
3. Record provider metadata in audit events (`provider`, `model`, `latency_ms`, `status`).
4. Implement safe fallbacks for provider timeout/rate-limit errors and map them to predictable decisions.
5. In tests, mock provider adapters and include at least one timeout/degraded-provider path.
...
3. Detailed technical teaching must include concise code snippets for critical runtime behaviors (for example fallback logic, routing logic, and safety decisions).
4. For week-oriented teaching tasks, create or update `docs/homework/week-XX.md`.
5. Homework must map directly to implemented code and tests in the same week.
6. Include runnable verification commands and expected outcomes.
7. Keep teaching/homework content operational and repository-specific, not generic theory.
