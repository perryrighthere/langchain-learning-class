...
5. For retrieval lessons, explicitly cover external embedding/rerank provider tradeoffs (quality, latency, cost, fallback).
6. Include one runnable provider-configured command when teaching practical integration.
7. For every critical behavioral claim (fallback, abstain/escalate, provider routing), include a concise code snippet showing how it is implemented in current modules.
8. When presenting a mode comparison (e.g., provider on/off), include runnable commands for both modes and what fields in output should differ.
...
5. `Detailed Technical Teaching` must include short code snippets tied to the exact module behavior being explained.
